...
export interface ForkedKernelHandle {
	readonly pid: number;
	/** Resolves when the fork server reaps this exact child or its connection dies. */
...
export class ForkServer {
	...
	private readonly childExitWaiters = new Map<number, () => void>();
	private readonly exitedChildren = new Set<number>();
	private dead = false;
	...
	private onData(chunk: string): void {
		...
		for (let idx = this.buffer.indexOf("\n"); idx !== -1; idx = this.buffer.indexOf("\n")) {
			...
			if (msg.type === "exit" && typeof msg.id === "number") {
				const resolve = this.childExitWaiters.get(msg.id);
				if (resolve) {
					this.childExitWaiters.delete(msg.id);
					resolve();
				} else {
					this.exitedChildren.add(msg.id);
				}
				continue;
			}
	...
	async spawnKernel(spawn: SpawnParams): Promise<ForkedKernelHandle> {
		...
		const exited = new Promise<void>((resolve) => {
			if (this.exitedChildren.delete(forkId) || this.dead) resolve();
			else this.childExitWaiters.set(forkId, resolve);
		});
		return {
			pid,
			exited,
	...
	private rejectPending(reason: string): void {
		...
		this.pending.clear();
		for (const resolve of this.childExitWaiters.values()) resolve();
...
function registerForkServerCleanupOnce(): void {
	...
	// KernelManager performs graceful signal and session shutdown. This synchronous
	// fallback is only for Node's non-awaitable final exit event.
