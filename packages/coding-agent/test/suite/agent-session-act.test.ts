...
import { fauxAssistantMessage, fauxToolCall, type TextContent, type Usage } from "@earendil-works/pi-ai";
...
function customMessage(content: string): CustomMessage {
	return {
		role: "custom",
...
class OrderedAbortActChannel extends AbortableActChannel {
	readonly receiveStarted: Promise<void>;
	private resolveReceiveStarted: () => void = () => {};

	constructor(private readonly events: string[]) {
		super();
		this.receiveStarted = new Promise((resolve) => {
			this.resolveReceiveStarted = resolve;
		});
	}

	override interruptAfterGrace(graceMs = 100): void {
		this.events.push("grace-interrupt");
		super.interruptAfterGrace(graceMs);
	}

	override receive(signal?: AbortSignal): Promise<Record<string, unknown>> {
		this.resolveReceiveStarted();
		signal?.addEventListener("abort", () => this.events.push("task-cancel"), { once: true });
...
function createTestMessageDeferred(): TestMessageDeferred {
...
type ActSessionInternals = {
	...
	_actLane?: {
		...
		running: boolean;
		steer(text: string): Promise<{ sequence: number }>;
	};
...
describe("AgentSession Act lane", () => {
	it("admits queued root prompts, compaction, and cells once in foreground order after Act cleanup", async () => {
		...
		const harness = await createHarness({
			...
			settings: { modelRoles: { luna: `${provider}/luna-model` } },
		});
		try {
	...
	it("delivers cell-active steering to the retained lane in order without waking Sol", async () => {
		...
		try {
			harness.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "steering_cell = 1" }), {
					stopReason: "toolUse",
				}),
				(context) => {
					...
					return fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "rlm.done(steering_cell)" }), {
						stopReason: "toolUse",
					});
				},
			]);
			...
			const act = internals._rootForeground.run("root-cell", async () => {
				...
				const exitAct = internals._rootForeground.enterAct(token);
				try {
					return await internals._runAct({ prompt: "steer the retained lane" }, undefined, channel);
				} finally {
					exitAct();
				}
			});
			...
			laneSession.agent.steeringMode = "all";

			const steeringMessageId = "agentmsg_act_steering_success";
			const delivery = harness.session.waitForAgentMessagePromptDelivery(steeringMessageId);
			const completion = createTestMessageDeferred();
			const outcome = internals._agentMessageOutcomes.get(steeringMessageId);
			if (!outcome) throw new Error("missing steering message outcome");
			outcome.completion = completion;
			const firstSteering = harness.session.steer("first lane steering", undefined, {
				agentMessageId: steeringMessageId,
				queueKey: "act-steering-key",
			});
			const secondSteering = harness.session.prompt("second lane steering", { streamingBehavior: "steer" });
			const sameKeySteering = harness.session.steer("same-key lane steering", undefined, {
				queueKey: "act-steering-key",
			});
			await vi.waitFor(() => expect(internals._actLane?.running).toBe(true));
			let queuedRootCells = 0;
			const queuedRootCell = internals._rootForeground.run("root-cell", async () => {
				queuedRootCells++;
			});
			expect(queuedRootCells).toBe(0);

			channel.complete(0, { type: "cell_result", stdout: "", stderr: "", result: "", error: null });
			await Promise.all([firstSteering, secondSteering, sameKeySteering]);
			...
			await expect(act).resolves.toEqual({ outcome: "done" });
			await queuedRootCell;
			expect(queuedRootCells).toBe(1);
			const steeringTexts = seenUserMessages.filter((text) => text.includes("lane steering"));
			expect(steeringTexts).toHaveLength(3);
			const persistedSteering = laneSession.sessionManager
				.getBranch()
				.filter((entry) => entry.type === "message" && entry.message.role === "user")
				.map((entry) =>
					entry.type === "message" && "content" in entry.message ? JSON.stringify(entry.message.content) : "",
				)
				.filter((text) => text.includes("lane steering"));
			expect(persistedSteering).toHaveLength(3);
			expect(steeringTexts[0]).toContain("first lane steering");
			expect(steeringTexts[1]).toContain("second lane steering");
			expect(steeringTexts[2]).toContain("same-key lane steering");
			expect(
				harness.session.messages.some(
					(message) => message.role === "user" && JSON.stringify(message.content).includes("lane steering"),
				),
			).toBe(false);
		} finally {
			harness.cleanup();
	...
	it("restores custom-message steering into the active lane without a root action", async () => {
		let restoredProviderTurns = 0;
		const harness = await createHarness({
			provider,
			models: [{ id: "sol-model" }, { id: "luna-model" }],
			settings: { modelRoles: { luna: `${provider}/luna-model` } },
		...
		try {
			harness.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "restored_steering_cell = 1" }), {
					stopReason: "toolUse",
				}),
				() => {
					restoredProviderTurns++;
					return fauxAssistantMessage(
						fauxToolCall("shared_ipython", { code: "rlm.done(restored_steering_cell)" }),
						{ stopReason: "toolUse" },
					);
				},
			]);
			const internals = harness.session as unknown as ActSessionInternals;
			const channel = new SequencedActChannel();
			const run = internals._runAct({ prompt: "restore custom steering" }, undefined, channel);
			await vi.waitFor(() => expect(channel.cells).toHaveLength(1));
			const laneSession = internals._actLane?.session;
			if (!laneSession) throw new Error("missing retained lane session");
			laneSession.agent.steeringMode = "all";
			const id = "agentmsg_act_restored_steering";
			const delivery = harness.session.waitForAgentMessagePromptDelivery(id);
			const completion = createTestMessageDeferred();
			const outcome = internals._agentMessageOutcomes.get(id);
			if (!outcome) throw new Error("missing restored steering outcome");
			outcome.completion = completion;
			const content: TextContent[] = [
				{ type: "text", text: "restored context" },
				{ type: "text", text: "restored custom steering" },
			];
			const restored = harness.session.restoreSteeringMessage("restored custom steering", undefined, {
				agentMessageId: id,
				queueKey: "restored-steering-key",
				content,
				prefixMessages: [customMessage("restored steering prefix")],
				customMessage: customMessage("restored steering primary"),
			});
			channel.complete(0, { type: "cell_result", stdout: "", stderr: "", result: "1", error: null });
			await restored;
			await expect(delivery).resolves.toBeUndefined();
			await expect(completion.promise).resolves.toBeUndefined();
			await vi.waitFor(() => expect(channel.cells).toHaveLength(2));
			...
			expect(restoredProviderTurns).toBe(1);
			const retainedCustom = laneSession.messages.filter(
				(message) => "customType" in message && message.customType === "prime-agent.act-steering-test",
			);
			expect(retainedCustom.map((message) => ("content" in message ? message.content : undefined))).toEqual([
				"restored steering prefix",
				"restored steering primary",
			]);
	...
	it("accepts steering submitted between shared cells into the next lane turn", async () => {
		let betweenSeen = false;
		const harness = await createHarness({
			provider,
			models: [{ id: "sol-model" }, { id: "luna-model" }],
			settings: { modelRoles: { luna: `${provider}/luna-model` } },
		...
		try {
			harness.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "between_cell_value = 1" }), {
					stopReason: "toolUse",
				}),
				(context) => {
					betweenSeen = context.messages.some(
						(message) =>
							message.role === "user" && JSON.stringify(message.content).includes("between-cell steering"),
					);
					return fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "rlm.done(between_cell_value)" }), {
						stopReason: "toolUse",
					});
				},
			]);
			const internals = harness.session as unknown as ActSessionInternals;
			const channel = new SequencedActChannel();
			const run = internals._runAct({ prompt: "steer between cells" }, undefined, channel);
			await vi.waitFor(() => expect(channel.cells).toHaveLength(1));
			channel.complete(0, { type: "cell_result", stdout: "", stderr: "", result: "1", error: null });
			const steering = harness.session.steer("between-cell steering");
			await steering;
			await vi.waitFor(() => expect(channel.cells).toHaveLength(2));
			expect(betweenSeen).toBe(true);
			channel.complete(1, { type: "done" });
			await expect(run).resolves.toEqual({ outcome: "done" });
		} finally {
			harness.cleanup();
	...
	it("rejects steering that loses the done terminal race without leaking it into the retained queue", async () => {
		const harness = await createHarness({
			provider,
			models: [{ id: "sol-model" }, { id: "luna-model" }],
			settings: { modelRoles: { luna: `${provider}/luna-model` } },
		...
		try {
			...
			const internals = harness.session as unknown as ActSessionInternals;
			const channel = new SequencedActChannel();
			const run = internals._runAct({ prompt: "finish while steering" }, undefined, channel);
			await vi.waitFor(() => expect(channel.cells).toHaveLength(1));
			const lane = internals._actLane;
			if (!lane?.session) throw new Error("missing retained lane");
			const steeringMessageId = "agentmsg_act_steering_terminal";
			const delivery = harness.session.waitForAgentMessagePromptDelivery(steeringMessageId);
			const completion = createTestMessageDeferred();
			const outcome = internals._agentMessageOutcomes.get(steeringMessageId);
			if (!outcome) throw new Error("missing terminal-race message outcome");
			outcome.completion = completion;
			const steering = harness.session.steer("terminal-race steering", undefined, {
				agentMessageId: steeringMessageId,
			});
			channel.complete(0, { type: "done" });
			await expect(steering).rejects.toThrow("no Act is active");
			await expect(delivery).rejects.toThrow("no Act is active");
			await expect(completion.promise).rejects.toThrow("no Act is active");
			await expect(run).resolves.toEqual({ outcome: "done" });
			await expect(lane.steer("after terminal")).rejects.toThrow("no Act is active");
			expect(lane.session.agent.hasQueuedMessages()).toBe(false);
			let ordinaryRootSteeringSeen = false;
			harness.appendResponses([
				(context) => {
					ordinaryRootSteeringSeen = context.messages.some(
						(message) =>
							message.role === "user" && JSON.stringify(message.content).includes("ordinary root steering"),
					);
					return fauxAssistantMessage("ordinary steering completed");
				},
			]);
			await harness.session.steer("ordinary root steering", undefined, { resumeIfIdle: true });
			await harness.session.waitForIdle();
			expect(ordinaryRootSteeringSeen).toBe(true);
			expect(
				lane.session.messages.some(
					(message) => message.role === "user" && JSON.stringify(message.content).includes("terminal-race"),
				),
			).toBe(false);
		} finally {
			harness.cleanup();
	...
	it("joins public abort ordering, steering cleanup, one terminal, and later root work", async () => {
		const events: string[] = [];
		const harness = await createHarness({
			provider,
			models: [{ id: "sol-model" }, { id: "luna-model" }],
		...
		try {
			harness.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "await integrated_cancel_work" }), {
					stopReason: "toolUse",
				}),
			]);
			const internals = harness.session as unknown as ActSessionInternals;
			const channel = new OrderedAbortActChannel(events);
			const run = internals._runAct({ prompt: "cancel through the root session" }, undefined, channel);
			await channel.cellSent;
			await channel.receiveStarted;
			const lane = internals._actLane;
			if (!lane?.session) throw new Error("missing active lane session");
			const requestProviderAbort = lane.session.requestAbort.bind(lane.session);
			vi.spyOn(lane.session, "requestAbort").mockImplementation(() => {
				events.push("provider-abort");
				requestProviderAbort();
			});
			const steering = harness.session.steer("steering cancelled with Act");
			...
			expect(events).toEqual(["provider-abort", "task-cancel", "grace-interrupt"]);
			expect(lane.session.agent.hasQueuedMessages()).toBe(false);
			let laterCellRuns = 0;
			await internals._rootForeground.run("root-cell", async () => {
				laterCellRuns++;
			});
			expect(laterCellRuns).toBe(1);
			harness.appendResponses([fauxAssistantMessage("later root prompt completed")]);
			await harness.session.prompt("later root prompt");
			expect(harness.session.getLastAssistantText()).toBe("later root prompt completed");
		} finally {
			harness.cleanup();
		}
	});

	it("settles provider-error and cancellation steering races at the typed boundary", async () => {
		...
		try {
			const failedEvents: ActProjectionEvent[] = [];
			failed.session.subscribe((event) => {
				if (event.type === "act_event") failedEvents.push(event);
			});
			failed.setResponses([
				fauxAssistantMessage("", { stopReason: "error", errorMessage: "steering provider boom" }),
			]);
			const internals = failed.session as unknown as ActSessionInternals;
			const run = internals._runAct({ prompt: "fail before steering" }, undefined, new TestActChannel());
			const lane = internals._actLane;
			if (!lane) throw new Error("missing active lane");
			const steering = failed.session.steer("provider-error steering");
			await expect(run).rejects.toThrow("steering provider boom");
			await expect(steering).resolves.toBeUndefined();
			expect(
				lane.session?.messages.some(
					(message) =>
						message.role === "user" && JSON.stringify(message.content).includes("provider-error steering"),
				),
			).toBe(true);
			expect(lane.session?.agent.hasQueuedMessages()).toBe(false);
			expect(failedEvents.map((event) => event.event)).toEqual(["start", "terminal"]);
			expect(failedEvents.at(-1)).toMatchObject({ event: "terminal", status: "error" });
			expect(JSON.stringify(failedEvents)).not.toContain("provider-error steering");
		} finally {
			failed.cleanup();
		}
		...
		try {
			const cancelledEvents: ActProjectionEvent[] = [];
			cancelled.session.subscribe((event) => {
				if (event.type === "act_event") cancelledEvents.push(event);
			});
			cancelled.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "await cancelled_steering_cell" }), {
					stopReason: "toolUse",
				}),
			]);
			const internals = cancelled.session as unknown as ActSessionInternals;
			const channel = new AbortableActChannel();
			const run = internals._runAct({ prompt: "cancel while steering" }, undefined, channel);
			await channel.cellSent;
			const lane = internals._actLane;
			if (!lane) throw new Error("missing active lane");
			const steering = lane.steer("cancel-race steering");
			...
			expect(
				lane.session?.messages.some(
					(message) => message.role === "user" && JSON.stringify(message.content).includes("cancel-race steering"),
				),
			).toBe(false);
			expect(cancelledEvents.map((event) => event.event)).toEqual([
				"start",
				"cell_start",
				"cell_terminal",
				"terminal",
			]);
			expect(cancelledEvents.filter((event) => event.event === "cell_terminal")[0]).toMatchObject({
				status: "cancelled",
			});
			expect(cancelledEvents.at(-1)).toMatchObject({ event: "terminal", status: "cancelled" });
			expect(JSON.stringify(cancelledEvents)).not.toContain("cancel-race steering");
			let staleSteeringSeen = false;
			cancelled.setResponses([
				(context) => {
					staleSteeringSeen = context.messages.some(
						(message) =>
							message.role === "user" && JSON.stringify(message.content).includes("cancel-race steering"),
					);
			...
			const nextChannel = new TestActChannel();
			await expect(
				internals._runAct({ prompt: "reuse after cancelled steering" }, undefined, nextChannel),
			).resolves.toEqual({ outcome: "done" });
			expect(staleSteeringSeen).toBe(false);
		} finally {
			cancelled.cleanup();
		}
	});
	...
	it("normalizes ordered assistant and shared-cell progress without private identities", async () => {
		...
		try {
			...
			harness.setResponses([
				...
				(context) => {
					steeringSeen = context.messages.some(
						(message) =>
							message.role === "user" && JSON.stringify(message.content).includes("projection cell steering"),
					);
					return fauxAssistantMessage([{ type: "text", text: "recover and finish" }, secondTool], {
						stopReason: "toolUse",
					});
				},
			]);
			...
			const steering = harness.session.steer("projection cell steering");
			channel.complete(0, {
				type: "cell_result",
	...
	it("restores the private transcript, closes one orphan once, and separates Luna usage", async () => {
		...
		const settings = { modelRoles: { luna: `${restartProvider}/luna-model` } };
	...
	it("routes update-restart shutdown into the active Act cancellation path", async () => {
		const harness = await createHarness({
			provider,
			models: [{ id: "sol-model" }, { id: "luna-model" }],
			settings: { modelRoles: { luna: `${provider}/luna-model` } },
		...
		try {
			harness.setResponses([
				fauxAssistantMessage(fauxToolCall("shared_ipython", { code: "await shutdown_cancel_work" }), {
					stopReason: "toolUse",
				}),
			]);
			const internals = harness.session as unknown as ActSessionInternals;
			const channel = new AbortableActChannel();
			const run = internals._runAct({ prompt: "cancel for update restart" }, undefined, channel);
			await channel.cellSent;
			const lane = internals._actLane;
			if (!lane?.session) throw new Error("missing active lane");
			const steeringError = harness.session.steer("shutdown-race steering").then(
				() => undefined,
				(error: unknown) => error,
			);
			harness.session.abortForUpdateRestart();
			await expect(steeringError).resolves.toMatchObject({ message: expect.stringContaining("no Act is active") });
			await expect(run).resolves.toEqual({ outcome: "cancelled" });
			expect(channel.interruptDelays).toEqual([100]);
			expect(lane.session.agent.hasQueuedMessages()).toBe(false);
			expect(harness.sessionManager.getBranch().filter((entry) => entry.type === "act_terminal")).toEqual([
	...
	it("fails explicitly when the Luna role is missing", async () => {
		const harness = await createHarness({ provider, models: [{ id: "sol-model" }] });
		try {
			const internals = harness.session as unknown as ActSessionInternals;
			await expect(internals._runAct({ prompt: "task" }, undefined, new TestActChannel())).rejects.toThrow(
				'Unknown RLM model role "@luna"',
			);
			expect((await harness.session.listRlmSubagents()).subagents).toEqual([]);
		} finally {
			harness.cleanup();
