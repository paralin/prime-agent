...
describe("auth migration ordering", () => {
	...
	it.each(["settings.json", "settings.yml"])(
		"keeps every credential source when the auth.json write fails with %s",
		(settingsName) => {
			const agentDir = makeAgentDir();
			const oauthPath = join(agentDir, "oauth.json");
			const settingsPath = join(agentDir, settingsName);
			writeFileSync(oauthPath, JSON.stringify({ anthropic: { access: "token" } }));
			writeFileSync(
				settingsPath,
				settingsName.endsWith(".yml")
					? "theme: dark\napiKeys:\n  openai: sk-key\n"
					: JSON.stringify({ theme: "dark", apiKeys: { openai: "sk-key" } }),
			);
			...
			expect(parseSettingsDocument(readFileSync(settingsPath, "utf-8")).apiKeys).toEqual({ openai: "sk-key" });
		},
	);
});
