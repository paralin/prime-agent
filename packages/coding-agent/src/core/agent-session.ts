...
export interface ActiveActSteeringRequest {
	text: string;
	images?: ImageContent[];
	content?: (TextContent | ImageContent)[];
	customMessage?: CustomMessage;
...
export interface ActiveActSteeringDelivery {
...
export class AgentSession {
	...
	private async _prompt(text: string, options?: InternalPromptOptions): Promise<void> {
		const actSteeringTarget =
			options?.streamingBehavior === "steer" && this._actLane?.running ? this._actLane : undefined;
		if (!this.isStreaming && !actSteeringTarget) {
			this._sessionInputPumpSuspended = false;
			this._assertSessionActionAdmissionAvailable();
		}
		const commitFence =
			this.isStreaming || actSteeringTarget
				? undefined
		...
		const run = async () => {
			try {
				throwIfPromptAdmissionCancelled(options?.signal);
				if (!actSteeringTarget) options?.admissionCommitted?.();
				...
				const normalizationResult = this._normalizeSubmission(text, options?.images, {
					parseSessionCommands: !actSteeringTarget && !isInternalPrompt && !options?.skipPrePromptWork,
					extensionCommands: actSteeringTarget ? "reject" : expandPromptTemplates ? "execute" : "ignore",
					inputSource:
						!actSteeringTarget && !isInternalPrompt && !options?.skipInputHandlers
				...
				if (actSteeringTarget) {
					if (normalized.kind !== "prompt") throw new Error("Act steering requires a prompt");
					try {
						await actSteeringTarget.steer(normalized.text, normalized.images);
						options?.admissionCommitted?.();
						reportPreflight(true);
						this._settleAgentMessage(options?.agentMessageId, "delivery");
						this._settleAgentMessage(options?.agentMessageId, "completion");
					} catch (error) {
						reportPreflight(false);
						this._rejectAgentMessage(options?.agentMessageId, this._asError(error));
						throw error;
					}
					return;
				}
	...
	/** Queue one Act steering message into the current private provider run. */
	enqueueActiveActSteering(request: ActiveActSteeringRequest): ActiveActSteeringDelivery {
		const content = request.content ?? this._buildPromptContent(request.text, request.images);
		const primary: QueuedAgentMessage = request.customMessage
			? cloneCustomMessage(request.customMessage)
			: {
					role: "user",
					content: content.map((block) => ({ ...block })),
					timestamp: Date.now(),
		...
		const messages: AgentMessage[] = [
			...(request.prefixMessages?.map((message) => cloneCustomMessage(message)) ?? []),
			primary,
		];
		const identities = new Set(messages);
		let settled = false;
		...
		const settle = (error?: Error) => {
			...
			settled = true;
			unsubscribe?.();
			if (error) rejectDelivery(error);
			else resolveDelivery();
		};
		unsubscribe = this.agent.subscribe((event) => {
			if (event.type === "message_start" && event.message === primary) {
				settle();
			} else if (event.type === "agent_end") {
				this.agent.removeQueuedMessages((candidate) => identities.has(candidate));
				settle(new Error("Act reached its terminal boundary before steering delivery"));
			}
		});
	...
	async steer(
		...
		const actSteeringTarget = this._actLane?.running ? this._actLane : undefined;
	...
	async restoreSteeringMessage(
		...
		if (actSteeringTarget) {
			try {
				await actSteeringTarget.steer(text, images, {
	...
	private async _runAct(
		...
		let sequence = 0;
		let started: { baseline: Usage; model: { provider: string; id: string } } | undefined;
		...
		try {
			const result = await lane.run(
				...
				(state) => {
					...
					started = { baseline: state.usage, model: state.model };
					this._emit({
						type: "act_event",
						actId,
						outerToolCallId,
				...
				(progress) => this._emitActProgress(actId, outerToolCallId, ++sequence, progress),
			);
	...
	private async _resolveActModel(model: string | undefined): Promise<NativeRlmSubagentModelSelection> {
		const reference = model ?? "@luna";
		const selection = await this._resolveRlmSubagentModel(reference);
		if (selection.runtime !== "native") {
			throw new Error(`Act model selector "${reference}" must resolve to a native model`);
		}
		return selection;
	}
