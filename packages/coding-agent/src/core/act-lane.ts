...
import type {
	ActiveActSteeringDelivery,
	ActiveActSteeringRequest,
	AgentSession,
	AgentSessionEvent,
...
export interface ActSteeringAcceptance {
	sequence: number;
}

export class ActSteeringRejectedError extends Error {
	constructor() {
		super("Act steering rejected because no Act is active");
		this.name = "ActSteeringRejectedError";
	}
}

interface ActiveSteering {
	sequence: number;
	request: ActiveActSteeringRequest;
	delivery?: ActiveActSteeringDelivery;
	resolve: () => void;
...
type BeforeActPrompt = (state: { usage: Usage; model?: { provider: string; id: string } }) => void;
...
interface ActiveAct {
	...
	steering: Map<number, ActiveSteering>;
...
function cloneSteeringCustomMessage(message: CustomMessage): CustomMessage {
	return {
		...message,
...
export class ActLane {
	...
	async run(
		...
		try {
			...
			const unsubscribeProgress = onProgress
				...
				: () => {};
			try {
				this.dispatchSteering(active, session);
	...
	async steer(
	...
	): Promise<ActSteeringAcceptance> {
		const active = this.active;
		if (this.disposed || !active?.acceptingSteering) throw new ActSteeringRejectedError();
		const sequence = ++active.steeringSequence;
		let resolve: () => void = () => {};
		...
		const steering: ActiveSteering = {
			...
			request: {
				...
				content: options.content?.map((block) => ({ ...block })),
				customMessage: options.customMessage ? cloneSteeringCustomMessage(options.customMessage) : undefined,
				prefixMessages: options.prefixMessages?.map((message) => cloneSteeringCustomMessage(message)),
			},
			resolve,
			reject,
			promise,
		};
		active.steering.set(sequence, steering);
		if (this.session) this.dispatchSteering(active, this.session);
		try {
			await promise;
			return { sequence };
		} finally {
			active.steering.delete(sequence);
		}
	}
	...
	private dispatchSteering(active: ActiveAct, session: AgentSession): void {
		for (const steering of active.steering.values()) {
			if (steering.delivery) continue;
			const delivery = session.enqueueActiveActSteering(steering.request);
			steering.delivery = delivery;
			void delivery.delivered.then(steering.resolve, (error: unknown) => {
				steering.reject(error instanceof Error ? error : new Error(String(error)));
			});
		}
	}

	private closeSteering(active: ActiveAct): void {
		if (!active.acceptingSteering && active.steering.size === 0) return;
		active.acceptingSteering = false;
		for (const steering of active.steering.values()) {
			const error = new ActSteeringRejectedError();
			if (steering.delivery) steering.delivery.cancel(error);
			else steering.reject(error);
		}
	}
