...
describe("kernel parent watchdog", () => {
	...
	it("a stale fork exit event after restart never tears down the new kernel", async () => {
		const journalPath = join(tempDir, "orphans.jsonl");
		process.env[ORPHAN_PROCESS_JOURNAL_ENV] = journalPath;
		...
		const handleB: ForkedKernelHandle = {
			pid: 222222,
			exited: new Promise(() => {}),
			isAlive: async () => true,
			kill: killB,
		...
		const internals = manager as unknown as {
			...
			forkedExitWatch?: ForkedKernelHandle;
			handleForkedKernelExit(handle: ForkedKernelHandle): void;
		};

		try {
			internals.state = "running";
			internals.forkedKernel = handleB;
			internals.forkedExitWatch = handleB;
			internals.handleForkedKernelExit(handleA);
	...
	it("the current fork exit event tears down the exited kernel", async () => {
		...
		try {
			...
			internals.forkedExitWatch = handle;
			internals.handleForkedKernelExit(handle);
