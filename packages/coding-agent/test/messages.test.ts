...
describe("addElapsedSystemPrompt", () => {
	...
	it.each([
		{ name: "omits the hint by default", settings: {}, expected: false },
		{
			name: "omits the hint when disabled",
			settings: { sessionElapsedTime: { enabled: false } },
			expected: false,
		},
		{
			name: "sends the hint when enabled",
			settings: { sessionElapsedTime: { enabled: true } },
			expected: true,
		},
	])("$name without adding a transcript message", async ({ settings, expected }) => {
		...
		try {
			...
			expect(providerSystemPrompt.includes("T+60s")).toBe(expected);
...
describe("convertToLlm repetition recovery", () => {
	it("sends the notice as a system-notice user turn and omits the cancelled loop", () => {
		const notice = createRepetitionNoticeMessage(10);
		const looping = {
			...fauxAssistantMessage("The roadmap file lives at the repository root. ".repeat(6), {
				stopReason: "error",
				errorMessage: "Repetition loop detected",
				timestamp: 9,
			}),
			diagnostics: [{ type: "agent_repetition_loop", timestamp: 9 }],
		};
		expect(convertToLlm([looping, notice])).toEqual([
			{
				role: "user",
