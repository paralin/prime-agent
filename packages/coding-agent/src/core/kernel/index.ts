...
export class KernelManager {
	...
	private forkedExitWatch?: ForkedKernelHandle;
	...
	private startForkedExitWatch(): void {
		const watched = this.forkedKernel;
		if (!watched) return;
		this.forkedExitWatch = watched;
		void watched.exited.then(() => this.handleForkedKernelExit(watched));
	}
...
// Keep the upstream subprocess client available for its isolated protocol tests
// while the production tool uses KernelManager for bidirectional Act comms.
export { ReplKernelManager } from "./repl-manager.js";
export type { KernelClient } from "./shared.js";
