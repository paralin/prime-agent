...
rlm.done(foreground_count)`,
	...
	it("routes steering into a cell-active Act on the managed kernel", async () => {
...
print(managed_steering_result)`,
	...
	it("restores completed lane context and root namespace after restart", async () => {
		...
		const settings = { modelRoles: { luna: `${provider}/luna-model` } };
...
try:
    ...
    print(${JSON.stringify(`${cancellation.label}-act-cancelled`)})`,
				...
				if (steeringError)
					await expect(steeringError).resolves.toMatchObject({
						message: expect.stringContaining("no Act is active"),
					});
			...
			harness.appendResponses([fauxAssistantMessage("root prompt succeeded after cancellation")]);
