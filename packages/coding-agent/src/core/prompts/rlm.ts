...
const IPYTHON_CONTROL_PROMPT = [
	...
	"",
	'Run shell commands from Python with `await bash("command")`. Pass `timeout=seconds` to terminate a command after a finite positive duration; omit it to allow the command to run until it exits or is cancelled. Use one command string for shell steps that depend on the same `cd`, environment variables, shell variables, or sourced files.',
	"",
