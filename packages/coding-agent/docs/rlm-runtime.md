...
Steering captured while Act is active enters the retained lane directly and resolves when its primary message enters that lane's ordered conversation. Addressed delivery and completion outcomes settle at the same boundary, and restored custom-message batches retain their prefix order. Steering does not admit a Sol turn or release the foreground lease. If the Act terminal wins the race, the targeted steering request fails instead of falling back to root steering. Ordinary steering is unchanged when no Act is active.
...
After the root journal records `act_start`, the session emits one additive projection bounded by `act_start` and `act_terminal`. Every event carries `actId`, the exact outer IPython `toolCallId`, and a monotonic Act-local `sequence`. `start` records the bounded prompt, resolved model, and cancellation capability. Only assistant thinking/text deltas and `shared_ipython` cell start/terminal facts appear between the boundaries. The self-contained terminal repeats the prompt, model, and capability and adds status, bounded error, and the selected model's usage. It is emitted after the journal terminal and contains no returned Python value or private transcript identity. The Act context-tree node reports the same current `model` and host `cancellationCapability`. Live events are not replayed; persisted journal, transcript, usage, and namespace restoration remain the recovery sources.
