...
describe("buildRlmPrompt", () => {
	...
	test("mentions external-event completion only when that skill is installed", () => {
		...
		expect(withoutExternalEvent).toContain(
			"A `bash()` handle left running beyond its creating cell sends a completion follow-up",
		);
	});
...
describe("buildSystemPrompt", () => {
	...
	test("formats the harness digest with refine guidance for REPL sessions", () => {
		...
		expect(prompt).toContain("Oversized fields are omitted whole");
		...
		expect(prompt).toContain("Results arrive through explicit `agent_message` replies or files");
		expect(prompt).toContain("await rlm.list_subagents()");
		expect(prompt).toContain("uses `child.session_name`");
	...
	test("keeps the harness digest compact", () => {
		...
		expect(prompt).toContain("[omitted: read the complete saved entry before using it]");
	...
	test("omits ipython-only subagent guidance when ipython is inactive", () => {
		...
		expect(digest).toContain("Call contract: use an installed skill as a shell command");
