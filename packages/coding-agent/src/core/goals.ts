...
If no concrete action is possible because progress depends only on an external actor or event, run \`await goal.pause("waiting for …")\` in ipython. A delegated subagent's reply counts as an external dependency: pause the goal instead of repeatedly polling the subagent. State the exact dependency and do not keep emitting holding updates. After new input resolves the blocker, run \`await goal.resume()\` before continuing the goal.
