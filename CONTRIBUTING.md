...
Add user-visible changes directly to the touched package's `CHANGELOG.md` under
the shared `## [Unreleased]` section. Add that section immediately below the
title when it does not exist. Use one short bullet per change, for example
`- Fixed the frobnicator dropping input on resize.` PRs that change
`packages/<pkg>/src` without updating that package's changelog fail CI; apply
the `no-changelog` label to opt out.
