...
export function buildChildAgentDoctrine(options: ChildAgentDoctrineOptions): string | undefined {
	...
	if (hasAgentMessage && hasIpython) {
		lines.push(
			'When a task calls for an answer, execute `await agent_message.send(message, receiver_role="parent")` in IPython. Do not merely write or quote that call as assistant text; text does not deliver the reply. Not every message or task needs a reply; continue cleanup after sending and go idle normally.',
		);
	}
	return lines.join("\n");
}
